...
from twitter_feed_scrapper.scrapping.scraping_utilities import ScrapingUtilities
...
class Finder:
    ...
    def find_replies(tweet) -> Union[int, str]:
        ...
        try:
            ...
            return ScrapingUtilities.extract_digits(replies)
        except Exception as ex:
    ...
    def find_shares(tweet) -> Union[int, str]:
        ...
        try:
            ...
            return ScrapingUtilities.extract_digits(shares)
        except Exception as ex:
    ...
    @staticmethod
    def find_timestamp(tweet) -> Union[datetime, None]:
        """finds timestamp from the tweet"""
        try:
            timestamp = tweet.find_element(By.TAG_NAME,
                                           "time").get_attribute("datetime")
            posted_time = datetime.strptime(timestamp[:-5], '%Y-%m-%dT%H:%M:%S')
            return posted_time
        except Exception as ex:
    ...
    def find_like(tweet) -> Union[int, None]:
        ...
        try:
            ...
            return ScrapingUtilities.extract_digits(likes)
        except Exception as ex:
    ...
    def is_retweet(tweet) -> bool:
        ...
        try:
            tweet.find_element(By.XPATH, './/a/span[text()=" Retweeted"]')
            return True
        except NoSuchElementException:
            return False
        except Exception as ex:
